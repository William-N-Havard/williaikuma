...
#
# -----------------------------------------------------------------------------
#   File: main.py (as part of project Recorder)
#   Created: 14/10/2022 02:49
#   Last Modified: 14/10/2022 02:49
# -----------------------------------------------------------------------------
...
from gui_utils import TkinterButtons as TkButtons
from utils import text_read, assert_recording_exists
...
class App:
    ...
    def _initiate_new_session(self):
        ...
        # Restore session ?
        already_existing_session = [item for item in os.listdir('.')
                                    if item.startswith('{}_{}'.format(self.speaker, data_source_filename))]
    ...
    def _get_recording_path(self):
        return os.path.join(self.session_path,
    ...
    def _set_sentence(self, index):
        ...
        self.Label_Frase['text'] = sent_text
        self.Label_ID['text'] = sent_id
...
if __name__ == "__main__":
    initial_dir = "/home/whavard/Documents/PROJECTS/TSIMANE/Fieldwork_2022/tsimane-sentences/sentences"

    root = tk.Tk()
    selected_file = filedialog.askopenfilename(initialdir=initial_dir, title="Elegir un archivo",
                                               filetypes=(("Text files", "*.txt*"),))
    if not selected_file:sys.exit(1)

    speaker = tkinter.simpledialog.askstring("Hablador?", "Quién está hablando?")
    if not speaker: sys.exit(1)

    app = App(root, data_source=selected_file, speaker=speaker)
    root.mainloop()
